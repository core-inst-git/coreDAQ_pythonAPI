...
class Controller(object):
    ...
    def __init__(self, idProduct, idVendor,check_motors=False):
...
# Commands Library implemented by Simone
    ...
    def move_indefinitely(self, motor, direction):
        if motor in np.arange(1,5):
            if direction == '+' or '-':
                self.command('%dMV%s'%(motor,direction))
            else:
                print('ERROR: Direction not Valid, please choose + or -')
        else:
            print('ERROR: Motor Code not Valid, please choose a number between 1 and 4')
    ...
    def get_acceleration(self):
        ...
        return float(rep.split('>')[1]) #step/sec2
    ...
    def get_speed(self):
        ...
        return float(rep.split('>')[1]) #step/sec
